...
function parseLanguage() {
    try {
        return JSON.parse(process.argv[9] || '{}')?.application?.language || 'en';
    } catch {
        return 'en';
    }
}
...
const ws = require('ws');
class Plugins {
    static language = parseLanguage();
    static globalSettings = {};
    static onShutdown = null;
    ...
    constructor() {
        ...
        this.ws.on('close', () => {
            try { Plugins.onShutdown?.(); } catch (_) { /* ignore */ }
            // Force-exit so StreamDock quit is not blocked by open handles (timers/log4js/child_process)
